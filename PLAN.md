...
(ingot :id "i2" :status molten :solo nil :grade 1 :skill cli :heat 0 :max 5 :smelt 0 :proof "cargo test --all 2>&1 | grep -q '0 failed'" :work "Run cargo test --all and verify all tests pass including the new extract_number_before empty string test, with 0 failures.")
