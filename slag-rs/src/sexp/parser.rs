...
const KNOWN_FIELDS: &[&str] = &[
    "id", "status", "solo", "grade", "skill", "heat", "max", "smelt", "proof", "bar", "work",
    "duel",
    "casts",
];
...
pub fn parse_ingot(line: &str) -> Option<Ingot> {
    ...
    let bar = get("bar").unwrap_or_default();
...
mod tests {
    ...
    #[test]
    fn a_bar_is_parsed_and_kept_out_of_extra() {
        let line = r#"(ingot :id "i1" :status ore :solo t :grade 1 :heat 0 :max 5 :proof "test -f calc.js" :bar "calc \"2+3*4\" prints 14" :work "calculator")"#;
        let i = parse_ingot(line).expect("parses");
        assert_eq!(i.proof, "test -f calc.js");
        assert!(i.bar.contains("prints 14"), "{}", i.bar);
        assert!(
            !i.extra.iter().any(|(k, _)| k == "bar"),
            "bar is a known field now, not forward-compat baggage"
        );
    }

    #[test]
    fn a_plan_without_a_bar_still_parses() {
        // Every crucible written before the field. They are judged by proof
        // alone rather than refused.
        let line = r#"(ingot :id "i1" :status ore :solo t :grade 1 :heat 0 :max 5 :proof "true" :work "w")"#;
        assert_eq!(parse_ingot(line).expect("parses").bar, "");
    }
    ...
    #[test]
    fn parse_duel_field_tristate() {
        let forced = parse_ingot(
            r#"(ingot :id "i1" :status ore :solo t :grade 1 :heat 0 :max 5 :proof "true" :work "w" :duel t)"#,
        )
        .unwrap();
        assert_eq!(forced.duel, Some(true));

        let blocked = parse_ingot(
            r#"(ingot :id "i2" :status ore :solo t :grade 5 :heat 0 :max 5 :proof "true" :work "w" :duel nil)"#,
        )
        .unwrap();
        assert_eq!(blocked.duel, Some(false));

        let absent = parse_ingot(
            r#"(ingot :id "i3" :status ore :solo t :grade 5 :heat 0 :max 5 :proof "true" :work "w")"#,
        )
        .unwrap();
        assert_eq!(absent.duel, None);
        // Known field: never leaks into extra.
        assert!(forced.extra.is_empty());
    }

    #[test]
    fn duel_field_roundtrips() {
        for line in [
            r#"(ingot :id "i1" :status ore :solo t :grade 4 :heat 0 :max 5 :proof "true" :work "w" :duel t)"#,
            r#"(ingot :id "i2" :status ore :solo t :grade 4 :heat 0 :max 5 :proof "true" :work "w" :duel nil)"#,
