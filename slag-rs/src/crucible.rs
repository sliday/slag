...
/// Process-wide lock serializing crucible load-modify-save sections.
/// Parallel anvils and the collector all rewrite the whole file; without
/// this, interleaved load/save pairs lose each other's updates.
pub static CRUCIBLE_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());
...
impl Crucible {
    ...
    /// Temp-file path for atomic saves.
    ...
    /// Save crucible to disk. Atomic: write a temp file, then rename, so a
    /// concurrent load never sees a half-written or truncated file.
    ...
    pub async fn save_async(&self) -> Result<(), std::io::Error> {
        let tmp = self.tmp_path();
        tokio::fs::write(&tmp, self.render()).await?;
    ...
    /// Reset stale Molten ingots back to Ore so they get rescheduled.
    /// Safe only when no anvil is actively working (e.g. at the top of the
    /// forge loop): any Molten status there is leftover from a panicked
    /// anvil or an interrupted previous run. Returns how many were reset.
    pub fn reset_stale_molten(&mut self) -> usize {
        let mut reset = 0;
...
"#
    ...
    #[test]
    fn reset_stale_molten_reschedules() {
        let f = write_temp(&sample_crucible());
        let mut c = Crucible::load(f.path()).unwrap();
        c.set_status("i1", Status::Molten);
        c.set_status("i3", Status::Molten);
        assert_eq!(c.reset_stale_molten(), 2);
        assert_eq!(c.get("i1").unwrap().status, Status::Ore);
        assert_eq!(c.get("i3").unwrap().status, Status::Ore);
        // Forged/cracked untouched, second pass is a no-op.
        assert_eq!(c.get("i2").unwrap().status, Status::Forged);
        assert_eq!(c.reset_stale_molten(), 0);
    }
    ...
    fn save_leaves_no_tmp_file() {
        ...
        c.save().unwrap();
        let tmp = c.tmp_path();
        assert!(!tmp.exists(), "tmp file left behind: {}", tmp.display());
        // Round-trip intact.
        let c2 = Crucible::load(f.path()).unwrap();
        assert_eq!(c2.ingots.len(), 4);
    }
